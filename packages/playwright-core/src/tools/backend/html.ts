...
const getPageHTML = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    ...
    try {
      ...
      response.addTextResult(`Original HTML length: ${html.length}.`);
      // Apply HTML cleaning using helper function
      html = await processHtmlCleaning(tab.page, html, cleaningParams);
      response.addTextResult(`Processed HTML length: ${html.length}.`);

      if (html.length > maxLength) {
        html = html.substring(0, maxLength) + '... [truncated]';
        response.addTextResult(`HTML content truncated to ${maxLength} characters:\n${html}`);
      } else {
        const lines = [];
